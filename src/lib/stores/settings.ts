...
function createSettingsStore() {
  ...
  return {
    ...
    updateTray(updates: Partial<TraySettings>) {
      update((s) => {
        const newTray = { ...s.app.tray, ...updates };
        const newApp = { ...s.app, tray: newTray };
        saveSettings(newApp);
        return { ...s, app: newApp };
      });
    },
    ...
    resetTray() {
      update((s) => {
        const newApp = { ...s.app, tray: defaultTraySettings };
        saveSettings(newApp);
        return { ...s, app: newApp };
      });
    },
...
export const isServerSettingsOpen = derived(
  settings,
  ($s) => $s.isServerSettingsOpen,
);
...
export const traySettings = derived(settings, ($s) => $s.app.tray);
