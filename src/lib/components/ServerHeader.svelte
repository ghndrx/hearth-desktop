<script lang="ts">
	...
	function toggleMenu() {
		showMenu = !showMenu;
		dispatch('toggleMenu', { isOpen: showMenu });
	}
...
<div class="server-header-wrapper">
	{#if server.banner}
		<div class="server-banner" style="background-image: url({server.banner})"></div>
	{/if}

	<button
		class="server-header"
		class:menu-open={showMenu}
		class:has-banner={server.banner}
	...
	{#if showMenu}
		...
		<div class="server-dropdown" on:click|stopPropagation>
			<button class="dropdown-item highlight-purple" on:click={handleInvitePeople}>
				<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
					<path d="M15 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm-9-2V7H4v3H1v2h3v3h2v-3h3v-2H6zm9 4c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>
				</svg>
				<span>Invite People</span>
				<svg class="external-icon" viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
					<path d="M15 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0-6c1.1 0 2 .9 2 2s-.9 2-2 2-2-.9-2-2 .9-2 2-2zm0 8c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4zm-6 4c.22-.72 3.31-2 6-2 2.7 0 5.8 1.29 6 2H9z"/>
				</svg>
			</button>

			<div class="dropdown-divider"></div>

			<button class="dropdown-item" on:click={handleOpenSettings}>
				<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
