...
describe('Panel lock state', () => {
	...
	it('should initialize panels with isLocked = false', () => {
		splitViewStore.pinDM(mockDMChannel);
		const panels = get(splitViewPanels);
		expect(panels[0].isLocked).toBe(false);
	});
	...
	it('should toggle lock state', () => {
		...
		expect(get(splitViewPanels)[0].isLocked).toBe(false);
		
		splitViewStore.toggleLock(panelId);
		expect(get(splitViewPanels)[0].isLocked).toBe(true);
		
		splitViewStore.toggleLock(panelId);
		expect(get(splitViewPanels)[0].isLocked).toBe(false);
	});

	it('should set lock state directly', () => {
		splitViewStore.pinDM(mockDMChannel);
		const panelId = get(splitViewPanels)[0].id;
		
		splitViewStore.setLocked(panelId, true);
		expect(get(splitViewPanels)[0].isLocked).toBe(true);
		
		splitViewStore.setLocked(panelId, false);
		expect(get(splitViewPanels)[0].isLocked).toBe(false);
	});

	it('should persist lock state', () => {
		splitViewStore.pinDM(mockDMChannel);
		const panelId = get(splitViewPanels)[0].id;
		splitViewStore.toggleLock(panelId);

		// Check that localStorage was called with isLocked
		const calls = localStorageMock.setItem.mock.calls;
		const panelCall = calls.find(call => {
			try {
				const data = JSON.parse(call[1]);
				return Array.isArray(data) && data[0]?.isLocked === true;
			} catch {
				return false;
			}
		});
...
describe('Edge case handling', () => {
	...
	const mockThread: Thread = {
		id: 'thread-edge',
		channel_id: 'channel-edge',
		parent_message_id: 'msg-edge',
		name: 'Edge Thread',
		message_count: 1,
		created_at: '2024-01-01T00:00:00Z'
	};
	...
	describe('Channel deletion handling', () => {
		it('should remove pinned channel when channel is deleted', () => {
			splitViewStore.pinChannel(mockServerChannel, 'server-edge');
			expect(get(splitViewPanels)).toHaveLength(1);
			
			splitViewStore.handleChannelDelete('channel-edge');
			expect(get(splitViewPanels)).toHaveLength(0);
		});

		it('should not affect other panels when channel is deleted', () => {
			const channel2: Channel = { ...mockServerChannel, id: 'channel-2' };
			splitViewStore.pinChannel(mockServerChannel, 'server-edge');
			splitViewStore.pinChannel(channel2, 'server-edge');
			expect(get(splitViewPanels)).toHaveLength(2);
			
			splitViewStore.handleChannelDelete('channel-edge');
			const panels = get(splitViewPanels);
			expect(panels).toHaveLength(1);
			expect(panels[0].channelId).toBe('channel-2');
		});
	});

	describe('Server removal handling', () => {
		it('should remove all panels for a server when server is removed', () => {
			const channel2: Channel = { ...mockServerChannel, id: 'channel-2' };
			splitViewStore.pinChannel(mockServerChannel, 'server-edge');
			splitViewStore.pinChannel(channel2, 'server-edge');
			expect(get(splitViewPanels)).toHaveLength(2);
			
			splitViewStore.handleServerRemove('server-edge');
			expect(get(splitViewPanels)).toHaveLength(0);
		});

		it('should not affect panels from other servers', () => {
			const channel2: Channel = { ...mockServerChannel, id: 'channel-other' };
			splitViewStore.pinChannel(mockServerChannel, 'server-edge');
			splitViewStore.pinChannel(channel2, 'other-server');
			expect(get(splitViewPanels)).toHaveLength(2);
			
			splitViewStore.handleServerRemove('server-edge');
			const panels = get(splitViewPanels);
			expect(panels).toHaveLength(1);
			expect(panels[0].serverId).toBe('other-server');
		});
	});

	describe('Thread deletion handling', () => {
		it('should remove pinned thread when thread is deleted', () => {
			splitViewStore.pinThread(mockThread, 'channel-edge', 'server-edge');
			expect(get(splitViewPanels)).toHaveLength(1);
			
			splitViewStore.handleThreadDelete('thread-edge');
			expect(get(splitViewPanels)).toHaveLength(0);
		});

		it('should not affect channel panels when thread is deleted', () => {
			splitViewStore.pinChannel(mockServerChannel, 'server-edge');
			splitViewStore.pinThread(mockThread, 'channel-edge', 'server-edge');
			expect(get(splitViewPanels)).toHaveLength(2);
	...
	describe('Non-existent ID handling', () => {
		it('should handle deletion of non-existent channel gracefully', () => {
			splitViewStore.pinChannel(mockServerChannel, 'server-edge');
			const panelsBefore = get(splitViewPanels);
			
			splitViewStore.handleChannelDelete('non-existent-channel');
			const panelsAfter = get(splitViewPanels);
		...
		it('should handle deletion of non-existent server gracefully', () => {
			...
			splitViewStore.handleServerRemove('non-existent-server');
			const panelsAfter = get(splitViewPanels);
		...
		it('should handle deletion of non-existent thread gracefully', () => {
			...
			splitViewStore.handleThreadDelete('non-existent-thread');
			const panelsAfter = get(splitViewPanels);
