<script lang="ts">
	...
	let { status = 'online', onOpenSettings }: Props = $props();
	...
	let discriminator = $derived(getDiscriminator($user?.id));
...
<div class="user-panel">
	<button class="user-info" onclick={openSettings}>
		<div class="avatar">
			{#if $user?.avatar}
		...
		<div class="user-details">
			<span class="username">{$user?.display_name || $user?.username || 'Guest'}</span>
	...
	<div class="controls">
		...
		>
			{#if isDeafened}
				<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
					<path
						d="M12 2C6.48 2 2 6.48 2 12v4c0 1.1.9 2 2 2h1v-6H4v-2c0-4.42 3.58-8 8-8s8 3.58 8 8v2h-1v6h1c1.1 0 2-.9 2-2v-4c0-5.52-4.48-10-10-10z"
					/>
		...
		<button class="control-btn" onclick={openSettings} title="User Settings">
			<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
				<path
